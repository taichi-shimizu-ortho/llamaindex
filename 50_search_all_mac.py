"""
[Mac版] 全論文横断RAG検索
Dropbox内の articles_all3.json からインデックスを読み込んで検索

使い方:
    python 50_search_all_mac.py "HMWHAの軟骨保護作用" "relaxinのMMP誘導"
"""
...
def load_index():
    ...
    if not STORAGE_DIR.exists():
        raise FileNotFoundError(
            ...
            f"（articles_all3.json のパス: {JSON_FILE}）"
        )
...
def main():
    print("=" * 80)
    print("全論文横断RAG検索 [Mac版 - articles_all3.json]")
    print("=" * 80)
    print(f"JSONソース: {JSON_FILE}")
    print(f"インデックス: {STORAGE_DIR.name}/")
    print("=" * 80)
