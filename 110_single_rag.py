"""
単一論文精読RAGシステム（インタラクティブモード）
MDファイルを直接読み込み、対話的にクエリを入力して検索結果をMarkdownファイルに追記
"""
...
# ══════════════════════════════════════════════════════════════════════════════
# コマンドライン引数でファイル名を指定（親フォルダは自動検索）
# ══════════════════════════════════════════════════════════════════════════════

# 使用例:
#   python 110_single_rag.py Em2015      → Dropbox/obsidian/10_article/*/Em2015.md を検索
#   引数なし → デフォルト (RXFP1/Em2015)

def find_md_file(filename: str) -> Path:
    """Dropbox/obsidian/10_article/ 以下から filename.md を検索"""
    base_dir = Path.home() / "Dropbox/obsidian/10_article"

    if not base_dir.exists():
        raise FileNotFoundError(f"検索ベースディレクトリが見つかりません: {base_dir}")
    ...
    if not matches:
        raise FileNotFoundError(f"{filename} が見つかりません (検索対象: {base_dir})")

    if len(matches) > 1:
        print(f"警告: {filename} が複数見つかりました:")
        for match in matches:
            print(f"  - {match}")
        print(f"最初のものを使用します: {matches[0]}")
...
if len(sys.argv) > 1:
    # ファイル名を指定
    filename = sys.argv[1]
    MD_FILE = find_md_file(filename)
else:
    # デフォルト
    MD_FILE = Path.home() / "Dropbox/obsidian/10_article/RXFP1/Em2015.md"
...
def md_to_structured_data(md_file_path: str) -> Dict[str, Any]:
    ...
    # Main Text 後の References セクションを抽出（## References の形式）
...
def split_references_into_items(text: str) -> List[str]:
    """Referencesセクションを1参考文献=1チャンクに分割
    対応形式:
    - Fujii2026: `- N\\.` または `- [N\\.` で始まる行
    - Em2015: `[N]` で始まる行
    """
    items = []
...
def main():
    ...
    base_path = Path.home() / f"Dropbox/obsidian/50_coding/llamaindex/{citekey}_{date_str}.md"
    ...
    print(f"ベクトルインデックス準備完了。対話的に質問を入力してください。")
    ...
    while True:
        ...
        print(f"✓ 質問 {query_count} を処理・追記しました\n")
