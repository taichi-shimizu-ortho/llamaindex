...
import { authorYearId, citationBaseId, uniqueJsonId } from "./citationId.js";
...
export async function harvestArticle(options: ArticleHarvestOptions): Promise<ArticleSet> {
  ...
  let finalDoi = "";
  if (options.jatsXml) {
    // JATSがある場合はXMLのタイトルを正とする。取得元ページがbot検査ページを返しても汚染されないようにする。
    const parsed = parseJatsArticle(options.jatsXml, { id: "dummy", sourceUrl, title: options.title?.trim() });
    if (parsed) {
  ...
  const finalTitle = set.title || title;
  // JATSに著者がある場合はそこからAuthorYear形式のIDを作る。
  // 取得元ページがbot検査ページを返すとHTML由来のIDが壊れるため。
  const jatsId = set.authors?.length ? authorYearId(set.authors[0], set.year ?? "") : "";
  const baseId = jatsId || citationBaseId(html || options.jatsXml || "", finalTitle, slugify(finalTitle || sourceUrl));
  const id = uniqueJsonId(PATHS.articleOutputDir, baseId, sourceUrl, finalDoi, finalTitle);
  set.id = id;
