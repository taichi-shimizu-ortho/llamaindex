...
function ImportPanel({
  ...
  onArticleImported: (articleId: string | undefined, folderId: string) => Promise<void>;
  onError: (message: string) => void;
}) {
  ...
  // DOIだけでPMC/Europe PMCのOA全文XMLを取れるか試す。
  // 取れなければ message に出版社URLの案内が入るので、ブックマークレット経路へ促す。
  async function fetchByDoi() {
    const doi = doiInput.trim();
    if (!doi || doiBusy || busy) return;
    setDoiBusy(true);
    setDoiMessage("");
    try {
      const result = await api.harvestByDoi(doi);
      if (result.ok) {
        setDoiInput("");
        setDoiMessage(`全文XMLを取得しました (source: ${result.source}${result.pmcid ? `, ${result.pmcid}` : ""})`);
        await onArticleImported(result.article?.id, folderId);
      } else {
        setDoiMessage(result.message || "全文XMLを自動取得できませんでした。");
      }
    } catch (e) {
      ...
      <div className="doi-fetch-panel">
        <label className="field">
          ...
          <div className="doi-fetch-row">
            ...
            />
            <button className="ghost" disabled={!doiInput.trim() || doiBusy || busy} onClick={fetchByDoi}>
              {doiBusy ? "Fetching..." : "Fetch by DOI"}
            </button>
          </div>
...
export function LibraryScreen({
  ...
  // 取り込み経路（HTML/URL・DOI）に関係なく、一覧更新→フォルダ振り分け→表示切替をまとめる。
  async function handleArticleImported(articleId: string | undefined, folderId: string) {
    await onRefresh();
    if (articleId && folderId) await run(() => api.assignDocument(articleId, folderId));
    if (articleId) setView(folderId || "unfiled");
  }

  async function handleImported(report: ImportReport, folderId: string) {
    await handleArticleImported(report.article?.id, folderId);
  }
