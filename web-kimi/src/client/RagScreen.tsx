...
export function RagScreen({ article, referenceSet, loading, onBackToLibrary }: RagScreenProps) {
  ...
  async function search() {
    ...
    try {
      ...
      const payload =
        searchMode === "article"
          ? { articleId: article?.id, query: query.trim(), topK }
          : searchMode === "integrated"
            ? { articleId: article?.id, referenceSetId: referenceSet?.id, query: query.trim(), topK }
            : { setId: referenceSet?.id, query: query.trim(), topK };
