...
// APIサーバが落ちているとViteのdevサーバがHTMLを返すため、JSON以外は
// 「APIに接続できていない」として扱い、パース失敗の不明なエラーにしない。
...
export const api = {
  ...
  // DOIだけを渡してPMC/Europe PMCのOA全文XMLを取得する。
  harvestByDoi: (doi: string) => postJson<DoiHarvestResult>("/api/article/harvest-by-doi", { doi }),
